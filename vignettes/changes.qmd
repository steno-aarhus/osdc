...
3.  Purchases of insulin in the previous year is no longer required for
    T1D classification (the vast majority of these individuals
    self-reported T1D in the Health in Central Denmark survey)
4.  The logic defining pregnancy index dates has been simplified to only
    ...
    care visits).
5.  HbA1c samples taken on the same date are de-duplicated (originally,
    if a sampling time was available in the lab data, only samples taken
    at the exact same time were de-duplicated)
...
This uses the same data as the paper and provides a direct comparison to
