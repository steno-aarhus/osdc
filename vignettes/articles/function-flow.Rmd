...
#' - `excluded_pregnancy`: Dates are the first and second elevated HbA1c test results (after censoring).
#' - `included_gld_purchases`: Dates are the first and second purchase of GLD of each individual.
#'
...
#' @param excluded_pregnancy Output from [exclude_pregnancy()].
#' @param included_gld_purchases Output from [include_gld_purchases()].
#'
#' @returns The same type as the input data, default as a [tibble::tibble()],
#'   with the joined columns from the output of [include_diabetes_diagnoses()],
#'   [include_podiatrist_services()], [exclude_pregnancy()], and
#'   [include_gld_purchases()]. There will be 1-8 rows per `pnr`.
#' @keyword internal
#' @inherit algorithm seealso
join_inclusions <- function(
    ...
    excluded_pregnancy,
    # TODO: Do we need this? There isn't an arrow from it in the diagram
    included_gld_purchases) {
  ...
  purrr::reduce(
    ...
    # TODO: I think full join works fine here, but may be computationally
    # expensive.
    # This joins *only* by pnr and dates. If datasets have the same column
    # names, they will be renamed to differentiate them.
    # TODO: We will need to ensure that no two datasets have the same columns.
