...
```{r}
#' Join the outputs from the inclusion and exclusion events.
#'
#' This function joins the outputs from all the inclusion and exclusion
#' events, by `pnr` and `dates`. From the input datasets:
#'
...
#'   [include_gld_purchases()]. There should be between 1 to 8 rows per `pnr`.
#'
...
`get_inclusion_date()` outputs a `data.frame` with the following
variables:
