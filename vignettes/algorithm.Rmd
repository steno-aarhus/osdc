...
are shown in the form of tables for each register, as well as for each
...
```{r}
algorithm()$lpr2_is_endocrinology |>
  as_tibble() |>
  knitr::kable()
```
...
```{r}
algorithm()$lpr3_is_endocrinology_department |>
  as_tibble() |>
  knitr::kable()
```
