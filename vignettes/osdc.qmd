...
The pre-installed R packages on Statistics Denmark's servers can be
quite old. This heavily limits the extent of `dplyr` operations
supported in `duckplyr` and `arrow` (for example, at the time of
writing, `library(duckplyr)` loads version 0.4 of the `duckplyr` package
from 2024, whereas the latest version on CRAN is 1.1 from 2026).

Fortunately, you can install the latest package from CRAN on DST's
virtual machine like normal: `install_packages("packagename")`. The only
downside is that you have to re-install your packages frequently: Every
week, when the servers are reset, and when logging in after having
logged out and terminated the previous DST session (in this case, you
may end up on a different R machine than the one you were using in the
previous session).

After making sure that you have the newest version of `osdc` (which
should install the updated dependencies) You can then load each register
directly from its Parquet folder and convert it to a DuckDB table,
without reading everything into memory. Despite the conversion from
Parquet/Arrow to DuckDB being very efficient, it's still a good idea to
perform some rough filtering of the data on the Arrow side before
converting to DuckDB, if you are working with nationwide raw data (most
of `dplyr`'s filtering operations are supported in Arrow, so you don't
have to change syntax):

``` r
diagnoser <- "path/to/diagnoser_parquet_folder" |>
  arrow::open_dataset(unify_schemas = TRUE) |>
  # Insert dplyr filtering steps here, as appropriate, e.g.:
...
If your data is already in R (e.g., as a data.frame object like
`diagnoser`) and fits in memory, you can convert it to a DuckDB table
with:
