...
of the `duckplyr` package from 2024, whereas the latest version on CRAN
is \>1.1 from 2026). These old versions don't support the `dplyr`
operations necessary for `osdc` to work.
...
without reading everything into memory. If you're working with very
large data sets, it's good practice to filter this data a bit on the
Arrow side before converting to DuckDB to reduce computation and memory
use. This is convenient, as most of `dplyr`'s filtering and selection
operations are supported by the `arrow` package. If you need to rename
variables or convert their types to match the inputs expected by `osdc`,
this is also an opportunity to do so.:
