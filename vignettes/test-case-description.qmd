...
-   **Objective**: To test the one logic path for T1D classification in
...
-   **Logic Tested**: The patient meets all criteria down the T1D path
    ...
    all GLD doses is less than two-thirds, causing the classification to
    fall through to T2D.
-   **Key Data Points**:
    -   `lmdb` records where the `volume` (DDD) of insulin purchases is
        less than twice the `volume` of non-insulin purchases.

#### Case 6: `06_t2d_oipF_endoT_majt1dT_i180F_itwo3T`

-   **Objective**: To test the failure of the "insulin within 180 days"
    rule (`i180F`).
-   **Logic Tested**: The patient has a majority of T1D diagnoses and a
    high proportion of insulin, but the first insulin purchase occurred
    more than 180 days after the first-ever GLD purchase. This fails an
    intermediate check, leading to a T2D classification.
-   **Key Data Points**:
...
-   **Objective**: To test the failure of the "majority T1D diagnoses"
    rule (`maj_t1dF`).
-   **Logic Tested**: The patient has qualifying diagnoses from an
    endocrinology department, but the number of primary T1D diagnoses is
    not strictly greater than the number of primary T2D diagnoses. This
    fails the majority rule, resulting in a T2D classification.
-   **Key Data Points**:
    -   An equal or lesser number of primary `DE10` diagnoses compared
        to primary `DE11` diagnoses in `diagnoser` linked to an
...
-   **Objective**: To test the T1D classification path for patients with
    diagnoses from non-endocrine medical departments.
-   **Logic Tested**: This case mirrors Case 4, but the qualifying
    diagnoses come from a valid medical department (e.g., `kardiologi`)
    instead of endocrinology. It confirms the logic `oipF` -\> `endoF`
    -\> `medT` -\> `maj_t1dT` -\> `i180T` -\> `itwo3T` works correctly.
-   **Key Data Points**:
    -   `kontakter` `hovedspeciale_ans` is a value from the valid
        medical department list.
    -   A majority of primary T1D diagnoses are linked to this contact.
    -   GLD purchase history in `lmdb` satisfies the `i180T` and
        `itwo3T` rules.
...
-   **Objective**: To test the failure of the `i180F` rule within the
    non-endocrine medical department branch.
-   **Logic Tested**: Mirrors Case 6. The patient's qualifying diagnoses
    are from a medical department, but they are classified as T2D
    because their first insulin purchase was too late.
-   **Key Data Points**:
    -   The `eksd` dates in `lmdb` for the first GLD and first insulin
        purchase are more than 180 days apart.

#### Case 11: `11_t2d_oipF_medT_majt1dF_i180T_itwo3T`

-   **Objective**: To test the failure of the `maj_t1dF` rule within the
    non-endocrine medical department branch.
-   **Logic Tested**: Mirrors Case 7. The patient does not have a strict
    majority of primary T1D diagnoses from relevant medical departments,
    leading to a T2D classification.
-   **Key Data Points**:
    -   An equal or lesser number of primary `DE10` diagnoses compared
        to primary `DE11` diagnoses in `diagnoser` linked to a medical
        department contact.
...
These cases test the very first step of the algorithm: identifying
...
-   **Logic Tested**: This patient has several records that look like
    inclusion events but should be ignored: a retracted diagnosis
    (`senere_afkraeftet = "Ja"`), a non-diabetes diagnosis code, a lab
    test that isn't for HbA1c, and a podiatrist service where the
    patient is the child of the service recipient (`barnmak = 1`).
    Despite having two valid HbA1c measurements, the `pnr` indicates
    this case is designed to test the filtering of invalid data from all
    sources.
-   **Key Data Points**:
    -   `diagnoser` record with `senere_afkraeftet = "Ja"`.
    -   `lab_forsker` record with a non-HbA1c `analysiscode`
        (`DNK35302`).
...
-   **Logic Tested**: The patient has exactly one qualifying podiatrist
    ...
    algorithm should find both, sum them to two, and include the
    patient.
-   **Key Data Points**:
...
-   **Objective**: To test that both old (`NPU03835`) and new
...
-   **Logic Tested**: The patient has one high HbA1c result using the
    ...
    both to reach the two-event threshold for inclusion. This case also
    includes pregnancy records that should not censor these events.
-   **Key Data Points**:
...
-   **Objective**: To test that diabetes diagnoses from both old
    (`lpr_diag`) and new (`diagnoser`) hospital registers are counted.
...
-   **Logic Tested**: The patient's only qualifying events are two
    separate purchases of glucose-lowering drugs. This confirms the
    baseline inclusion logic for the medication register. This case also
    includes pregnancy records that should not censor these events.
-   **Key Data Points**:
...
that occur under specific circumstances (e.g., during pregnancy). These
individuals should be classified as `nodm` because their only potential
inclusion events are censored.

#### Case 17: `17_nodm_glp1a_dapa_empa`

-   **Objective**: To test the censoring of specific GLDs (GLP-1 RAs,
    SGLT2i) that may be used for indications other than diabetes.
-   **Logic Tested**: The patient has multiple drug purchases with ATC
    codes starting in `A10BJ` or `A10BK`. The algorithm should identify
    and ignore these, leaving the patient with zero inclusion events.
-   **Key Data Points**:
    -   All `lmdb` records have ATC codes matching the exclusion list
        (e.g., `A10BJ01`, `A10BK01`, `A10BK03`).
...
-   **Logic Tested**: The patient is a female under 40, and her only
...
-   **Objective**: To test that metformin purchases are censored if they
    ...
    40. 
-   **Logic Tested**: The patient is a female over 40, but her metformin
    purchases have `indo` codes that are on the PCOS indication list.
    These events should be censored, resulting in a `nodm`
    classification.
-   **Key Data Points**:
    -   `bef` record for a female \> 40 years old.
    -   `lmdb` records for metformin (`A10BA02`) with `indo` values from
        the PCOS list (e.g., `'0000092'`).
...
-   **Logic Tested**: The patient has a recorded pregnancy-ending event.
    Her only high HbA1c and GLD purchase occur within the window of 40
    weeks before to 12 weeks after this event. Both should be censored,
    leaving her with zero inclusion events.
-   **Key Data Points**:
    -   A pregnancy-related diagnosis in `diagnoser` (e.g., `DO806`) on
        a specific date.
    -   `lab_forsker` and `lmdb` records with `samplingdate` and `eksd`
        falling within the calculated pregnancy window around that
        diagnosis date.
...
-   **Logic Tested**: This patient exists in the base population (`bef`)
    ...
    process this individual without error and correctly determine they
    have zero inclusion events, resulting in a `nodm` classification.
-   **Key Data Points**:
    -   A record in `bef`.
