...
virtual machines of Statistics Denmark. The package does not support inputs in their raw `.sas7bdat` format,
and these must be converted to ´.parquet` files first, which can then be loaded in as DuckDB objects. The [`registers2parquet` package](https://dp-next.github.io/registers2parquet/) provides functions to conveniently handle this.
