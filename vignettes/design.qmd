...
- We *do not* assume that the register data comes directly from the
  original source (SAS) files. In fact, many functions assume that a
  certain level of pre-processing and cleaning has been already done to
  the data. These assumptions will be checked, so if the data does not
  meet these assumptions, the user will be informed.
- Related to the above, we also assume the data has already been loaded
  from the original source files and joined into a single data frame for
  any given register. Denmark Statistics stores their register data by year,
  but we require that all years for a register are joined into one.
...
- Function arguments take a single DuckDB type object as input (e.g.
  `duckplyr_df`). This also means that the register datasets must be
  loaded from the multiple source files and joined into one data frame
  for any given argument that requires that register.
