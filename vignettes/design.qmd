...
-   **Sufficient data on inclusion events:** while HbA1c test results
    ...
    nationwide: from Q4 2015 onward, according to [this
    analysis](https://ndownloader.figstatic.com/files/42763726).
    However, as the vast majority of diabetes patients are treated with
    glucose-lowering drugs at some point, we considered the availability
    of data on purchases of prescription drugs sufficient. These are
...
-   **Sufficient data on exclusion events:** in order to correctly
    ...
    to gestational diabetes, rather than manifest diabetes, register
    information on pregnancy occurances is necessary. In the patient
    register, this information is available from 1994 onward, but is not
    valid until 1997, according to [this
...
-   **Sufficient wash-out period:** In order to "wash out" prevalent
    ...
    considered a full year to be enough. Thus, only inclusion dates
    occuring from 1998 onward are considered true incident cases, and
    have a `stable_inclusion_date` value.

<!-- TODO: Specify that using the maternal pregnancy register, you can (probably) create a stable population a few years earlier -->
