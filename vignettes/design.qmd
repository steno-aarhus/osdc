...
In order to classify diabetes status correctly, we need the patient
registers for diagnosis information (known collectively as LPR). And
there isn't just one LPR, there are actually several different LPR
datasets that have evolved over time as needs change. Denmark Statistics
in fact relatively recently created a new LPR (LPR3A) that resolves some
issues with the previous LPR registers. Not only are there several LPRs,
but each new version also has several different tables and variables,
though osdc only needs specific variables from two tables.
...
and joined first before entering into `classify_diabetes()`. This way,
we can just add a new pre-processing function for any new changes to LPR
without changing the interface of `classify_diabetes()`.
...
`prepare_lpr2()`. This way, if they update the LPR again, we can just
add another `prepare_lpr*()` function to prepare the new LPR format
without changing the interface of `classify_diabetes()`.
...
`is_medical_dept`, and `is_pregnancy_code`. See the help docs for
...
that prepare them for the main function.
